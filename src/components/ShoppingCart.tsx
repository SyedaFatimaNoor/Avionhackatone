const ShoppingCart = () => {
    ...
    return (
      <section className="p-8">
        <h1 className="text-2xl font-semibold text-[#2A254B] mb-8">Your shopping cart</h1>
        <div className="w-full border-b border-gray-300 mb-4">
          <div className="grid grid-cols-4 text-sm font-medium text-[#505977]">
            <span>Product</span>
            <span>Quantity</span>
            <span>Total</span>
          </div>
        ...
        {cartItems.map((item) => (
          <div key={item.id} className="grid grid-cols-4 items-center py-4 border-b border-gray-200">
            <div className="flex items-start">
              ...
              />
              <div>
                <h2 className="text-base text-[#2A254B]">{item.title}</h2>
                <p className="text-sm text-[#505977]">{item.description}</p>
                <span className="text-sm font-medium text-[#2A254B]">£{item.price}</span>
              </div>
            </div>
            <div className="text-center">{item.quantity}</div>
            <div className="text-right text-[#2A254B] font-medium">£{item.price * item.quantity}</div>
          </div>
        ))}
        ...
        <div className="mt-8">
          <button className="px-6 py-3 bg-[#2A254B] text-white text-sm font-medium rounded hover:bg-[#3a3475]">
            Go to checkout
          </button>
        </div>
      </section>
